 ...
 * @author Ben Wirz
 * 
 * @brief The module is is a cooperative task scheduler which provides an 
 * easy-to-use mechanism for scheduling tasks to be executed in the future 
 * without the complexity or overhead of an operating system.  Once scheduled, 
 * a task's handler is executed from the main context after its interval timer 
 * expires. 
 */
 ...
 *
 * An unbuffered task can also be used for tasks which do  not need to pass 
 * data to their handlers.  For example, an LED blink task may not need to 
 * store task data since it can simple invert the LED output during the handler 
 * call.
 * 
 * @note Since this macro statically allocates a task, it should only be invoked 
 * once per task.
 * 
 ...
 * internal data buffer is used to pass a copy of the user supplied data to the 
 * task handler.  The buffer size is configurable on per task basis and should 
 * be set the maximum data size which will be stored in the task.
 *
