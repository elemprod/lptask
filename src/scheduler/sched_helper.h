                                ...
                                uint8_t ms)
{
  return ((days * SCHED_MS_DAY) + (hours * SCHED_MS_HOUR) +
          (mins * SCHED_MS_MINUTE) + (secs * SCHED_MS_SECOND) + ms) &
         UINT32_MAX;
}
...
static inline bool sched_task_buffered(const sched_task_t *p_task)
{
  ...
  {
    // Buffered tasks have non-zero buffer sizes.
 ...
 *
 * Note that a NULL or inactive task will return false since it can't expire.
 *
 * @param[in] p_task  Pointer to the task to stop to the scheduler
 *
 * @retval True if task's timer has expired.
 * @retval False if the task pointer was NULL or the task is unexpired.
 */
bool sched_task_expired(const sched_task_t *p_task);

/**
 * @brief Function for getting the time until a task's timer expires.
 *
...
/**
 * @brief Function for getting the time interval since the task was started.
 *
 * Note that repeating tasks are restarted during each handler call.  The value
 * returned by the function will be the time interval since the last handler
 * call for a repeating task which has previously expired.
 *
 * @param[in] p_task  Pointer to the task.
 *
 * @retval The time interval in mS since the task was started.
...
/**
 * @brief Function for determining the number of allocated tasks in a
 * scheduler task pool.
 *
...
/**
 * @brief Function for determining the number of unallocated (available) tasks
 * in a scheduler task pool.
 *
 * @param[in] p_pool  Pointer to pool configuration structure.
 *
